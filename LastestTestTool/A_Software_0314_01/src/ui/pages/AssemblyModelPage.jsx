...
import {
  ...
  initializeHardwareStore, 
  //0319
  startAssemblyTeachMode, // 确保导入了这个函数
...
export default function AssemblyModelPage({ onGoExecution }) {
  ...
  //0319 --- 新增：监听硬件连接成功并发送 U 和 K 指令 ---
  useEffect(() => {
    // 只有当 connection 变为 'connected' 且 source 变为 'hardware' 时才触发
    if (hardware.connection === 'connected' && hardware.source === 'hardware') {
      console.log("检测到真机连接成功，正在发送初始化指令 (U: 卸力, K: 坐标流)...");
      
      // 调用之前在 useHardwareStore.ts 中定义的函数
      // 该函数会发送 'UNLOCK' (U) 和 'TOGGLE_COORD' (K)
      import('../../services/useHardwareStore.ts').then(m => {
        m.startAssemblyTeachMode();
      });
    }
  }, [hardware.connection, hardware.source]); 
  // 依赖项：当连接状态或来源发生变化时，React 会重新检查上面的 if 条件
