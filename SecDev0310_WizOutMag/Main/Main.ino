...
void loop(){
    loop_nled();
    //切换LED灯状态实现LED灯闪烁 [cite: 344, 345]
    loop_uart();    //解析串口接收到的字符串指令 [cite: 345]
    loop_action();  //循环执行是否需要读取数据执行动作组 [cite: 345]
    loop_servo();   //处理模拟舵机增量 [cite: 345]
    loop_Function(); 
    PS2_controll(); 
    // stream_servo_positions_to_pc(); // 建议默认注释掉，避免干扰示教时的读取 [cite: 346]
