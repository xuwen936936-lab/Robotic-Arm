...
export default function AssemblyModelPage({ onGoExecution }) {
  ...
  const [waypoints, setWaypoints] = useState([])
  const [waypointFrames, setWaypointFrames] = useState([])
  ...
  // 2. 坐标系切换通知底层
  useEffect(() => {
    hardware.setReferenceFrame(referenceFrame)
  }, [referenceFrame])

  // 3. 继电器控制
  useEffect(() => {
    hardware.controlMagnet(mode === 'pick')
  }, [mode])

  // 4. 监听硬件弹窗信号
  useEffect(() => {
    const unsubscribe = subscribeHardwareSignal((signal) => {
      if (signal === HARDWARE_SIGNALS.ASSEMBLY_REACHED_SPECIFIED_POINT) {
        setCollisionHintType('waypoint')
        setCollisionHintStep(1)
        setShowCollisionHintModal(true)
        setHasCollision(true)
      } else if (signal === HARDWARE_SIGNALS.ASSEMBLY_ESTOP_BEFORE_TARGET) {
        setCollisionHintType('direction')
        setCollisionHintStep(1)
        setShowCollisionHintModal(true)
        setHasCollision(true)
      } else if (signal === HARDWARE_SIGNALS.ASSEMBLY_SINGULARITY_REACHED) {
  ...
  // ==========================================
  // 🛡️ 恢复的 Mock 与 UI 交互逻辑 
  // ==========================================
  const handlePointChange = (setter, axis, value) => {
    setter((prev) => ({ ...prev, [axis]: value }))
  }

  const handleWaypointChange = (index, axis, value) => {
    setWaypoints((prev) => {
      const newWp = [...prev]
      newWp[index] = { ...newWp[index], [axis]: value }
      return newWp
    })
  }

  const handleJogMove = (command) => {
    // 兼容没有连硬件时，可以通过前端 UI 面板虚拟移动坐标
    if (hardware.sendMockJogMove) {
      hardware.sendMockJogMove(command)
    }
  }
  ...
  const handleRecordWaypoint = (index) => {
    setWaypoints((prev) => {
      const newWp = [...prev]
      newWp[index] = hardware.captureCurrentPoint()
      return newWp
    })
  }

  const handleAddWaypoint = () => {
    setWaypoints((prev) => [...prev, EMPTY_POINT])
    setWaypointFrames((prev) => [...prev, 'Base'])
  }

  const handleRemoveWaypoint = (index) => {
    setWaypoints((prev) => prev.filter((_, i) => i !== index))
    setWaypointFrames((prev) => prev.filter((_, i) => i !== index))
  }

  const handleTestClick = () => {
    if (hardware.isRunning) return
    if (!isPointFilled(grab) || !isPointFilled(drop)) {
      setShowToast(true)
      return
    }
    hardware.executeAssemblyPath(grab, waypoints, drop, referenceFrame)
  }
  ...
  return (
    <AssemblyModelPageView
      ...
      showRelativeHintInfo={showRelativeHintInfo}
      showWrongAnswerToast={showWrongAnswerToast}
      onCloseCollisionHint={() => setShowCollisionHintModal(false)}
      onSelectCollisionOption={setSelectedCollisionOption}
      onConfirmCollisionHint={() => {
        if (collisionHintType === 'direction' && collisionHintStep === 1) {
          setCollisionHintStep(2)
      ...
      onChangeDropFrame={setDropFrame}
      onChangeWaypointAxis={handleWaypointChange}
      onChangeWaypointFrame={(idx, val) => {
        setWaypointFrames(prev => {
          const newF = [...prev]; newF[idx] = val; return newF;
        })
      }}
      onChangeJogFrame={setJogFrame}
      onJogMove={handleJogMove} 
      ...
      onRecordWaypoint={handleRecordWaypoint}
      showAddWaypoint={waypoints.length < MAX_WAYPOINTS}
      onPrimaryAction={handleTestClick} 
    />
  )
}