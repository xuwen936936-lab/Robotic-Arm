...
### 4) Singularity reached (block 3)

- Signal name: `ASSEMBLY_SINGULARITY_REACHED`
- Trigger timing:
  - Assembly 第三块流程运行中
  - 硬件检测到 TCP 路径到达奇异点并停止运动时发送
- Purpose:
  - 停止运行状态，触发奇异点异常弹窗与后续处理流程
...
2. WebSocket JSON event (gateway -> frontend):
   ...
   - `{"type":"event","event":"ASSEMBLY_SINGULARITY_REACHED"}`
