...
void loop_fixed_move() {
  ...
  if (Serial.available() > 0) {
    ...
    if (key == 'F') {
      ...
      smart_delay_with_stop(FIXED_MOVE_TIME); // 阻塞等待机械臂走到初始位置
      ...
      smart_delay_with_stop(FIXED_MOVE_TIME); // 阻塞等待机械臂走到起点
      ...
      smart_delay_with_stop(500); // 给 0.5 秒缓冲时间让磁铁吸稳物品
      ...
      smart_delay_with_stop(FIXED_MOVE_TIME); // 阻塞等待机械臂走到终点
      ...
      digitalWrite(RELAY_PIN, LOW);
      smart_delay_with_stop(500); // 给 0.5 秒缓冲时间让物品自然掉落
      ...
      smart_delay_with_stop(FIXED_MOVE_TIME); // 阻塞等待复位完成
